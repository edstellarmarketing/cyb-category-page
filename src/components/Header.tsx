...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-eds-gray-200 bg-white">
      <div className="eds-page-center flex h-16 items-center justify-between">
          ...
          {NAV.map((item) => (
            ...
            >
              ...
              {item.children && openDropdown === item.label && (
                <div className="absolute top-full left-0 z-50 min-w-[240px] rounded-md border border-eds-gray-200 bg-white py-3 shadow-lg">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            ...
            {langOpen && (
              <div className="absolute right-0 top-full z-50 mt-2 w-[200px] rounded-md border border-eds-gray-200 bg-white py-2 shadow-lg">
                {LANGUAGES.map((lang) => (
                  <button
                    key={lang}
                    type="button"
      ...
      {searchOpen && (
        <div className="border-t border-eds-gray-200 bg-white">
          <div className="eds-page-center py-4">
            <input
              type="search"
              autoFocus
              placeholder="Search training programs, or trainers..."
              className="w-full rounded-md border border-eds-gray-300 px-4 py-3 text-[16px] outline-none focus:border-[#6366F1]"
            />
          </div>
        </div>
      )}
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 bg-white lg:hidden">
          <div className="flex h-16 items-center justify-between border-b border-eds-gray-200 px-6">
            <span
              className="text-[20px] italic tracking-wider text-[#6366F1]"
              style={{ fontFamily: "'Riona Sans Bold', Helvetica, Arial, sans-serif", fontWeight: 800 }}
            >
          ...
          <nav className="px-6 py-6">
            {NAV.map((item) => (
              <div key={item.label} className="border-b border-eds-gray-200 py-4">
                <Link
                  href={item.href}
